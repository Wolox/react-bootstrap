const generateRARScript = (command, options) => `react-app-rewired ${command} --scripts-version wolox-react-scripts ${options}`;

const getPackageJsonAttributes = (
  projectName,
  projectVersion,
  repoUrl
) => 
...
module.exports = function configPackageJson() {
  ...
  const newpjson = Object.assign(pjson, getPackageJsonAttributes(
    this.projectName,
    "1.0.0",
    this.repoUrl
  ));
